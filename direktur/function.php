...
function acc_direktur($id_sp) {
	global $conn;
	$tgl = date('Y-m-d');

	$query ="UPDATE tb_pengajuan SET status = '4', tgl_direktur= '$tgl' WHERE id_sp = $id_sp";

	mysqli_query($conn,$query);
	return mysqli_affected_rows($conn);	
	
}
function dec_direktur($id_sp) {
	global $conn;
	

		//insert data
	$query ="UPDATE tb_pengajuan SET status = '5' WHERE id_sp = $id_sp
	";
